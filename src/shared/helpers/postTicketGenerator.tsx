...
export const POSTicketGenerator = ({
  ...
  const generateTicketText = (): string => {
    ...
    let ticketText = `
${center('ZONA CONEXION', 50)}
${center('COMOAPA, BOACO', 50)}
${center('(505) 77041065', 50)}
${lineBreak}
...
${products
  .map((product) => {
    ...
    return productLines
      .map((line, index) =>
        index === 0
          ? `${line.padEnd(30)}${`x${product.quantity}`.padStart(6)}${product.price.toFixed(2).padStart(12)}`
          : `${line.padEnd(30)}`
      )
      .join('\n');
  })
...
${lineBreak}
EFECTIVO: ${`C${efectivoRecibido}`.padStart(35)}
CAMBIO: ${`C$${change}`.padStart(38)}
${totalDiscount ? `DESCUENTO TOTAL: ${`C$${totalDiscount}`.padStart(27)}` : ''}
TOTAL: ${`C${total}`.padStart(41)}
${lineBreak}
${space}
${space}
${center('Gracias por su Preferencia', 50)}
`;
    return ticketText;
  };
